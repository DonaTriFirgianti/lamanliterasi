...
@section('content')
    <div class="bg-pink-50 rounded-2xl shadow-md p-8">
        <h2 class="text-2xl font-bold text-purple-700 mb-6">Edit Barang</h2>
        ...
        <form action="{{ route('admin.items.update', $item->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label class="block mb-2 text-sm font-medium text-purple-700">Jumlah Stok</label>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4" required
                        class="w-full rounded-lg border border-pink-200 focus:border-purple-400 focus:ring-purple-300 shadow-sm">{{ old('description', $item->description) }}</textarea>
                    @error('description')
                        <p class="text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
